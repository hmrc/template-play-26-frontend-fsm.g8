...
case class $servicenameCamel$ApiRequest(
  ...
  def toUpperCase: $servicenameCamel$ApiRequest =
    copy(givenName = this.givenName.toUpperCase, familyName = this.familyName.toUpperCase)

}

object $servicenameCamel$ApiRequest {
  implicit val formats = Json.format[$servicenameCamel$ApiRequest]
